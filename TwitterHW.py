...
#TWITTER_URL = 'https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=umsi'
...
##YOUR CODE HERE
    ...
    results = api.user_timeline(screen_name ="umsi")
    #list_of_umsi_tweets = results["statuses"]
    #tweetjson = json.dumps(list_of_umsi_tweets, indent=4) #encode the python list object to JSON string
    tweetjson = json.dumps(results, indent=4) #encode the python list object to JSON string
    ...
    if "umsi" in CACHE_DICTION:
        ...
        try:
            ...
            CACHE_DICTION["umsi"] =  json.loads(tweetjson)
...
cur = conn.cursor()
# 2 - Write code to drop the Tweets table if it exists, and create the table
#(so you can run the program over and over), with the correct (4) column names and appropriate types for each.
...
print(type(umsi_tweets),"is the type of the list of term tweets")
...
#    cur.execute('''insert OR IGNORE INTO Tweets(tweet_id, author, time_posted, tweet_text, retweets) Values(?,?,?,?,?)''',
#            (tweet.get('id'), (tweet.get('user')).get('name'), tweet.get('created_at'),  tweet.get('text'), tweet.get('retweet_count')))
...
for row in cur:
    print(row) #for Mac
    #print(row[0], row[1].encode('UTF-8')) #for PC
...
# assume conn and cursor
#cur.execute("SELECT time_posted, tweet_text FROM Tweets")
#all_res = cur.fetchall()
#for t in all_res:
    #print(t[0] + " - " + t[1] + "\n") #for Mac
#    print(t[0] + " - " + t[1].strip().encode('UTF-8') + "\n") #for PC
